...
// ---------- ws ----------
let ws;
function setDebugBanner(s, color) {
  let b = document.getElementById("ws-banner");
  if (!b) {
    b = document.createElement("div");
    b.id = "ws-banner";
    b.style.cssText = "position:fixed;top:0;left:50%;transform:translateX(-50%);z-index:9999;padding:4px 12px;border-radius:0 0 6px 6px;font-size:11px;font-weight:700;letter-spacing:0.1em;";
    document.body.appendChild(b);
  }
  b.textContent = "WS: " + s;
  b.style.background = color || "#7ad7ff";
  b.style.color = "#021018";
  console.log("[WS]", s);
}
function connect() {
  setDebugBanner("connecting...", "#ffd47a");
  try { ws = new WebSocket(`ws://${location.host}/ws`); }
  catch (e) { setDebugBanner("construct failed: " + e.message, "#ff5d8f"); setTimeout(connect, 2000); return; }
  ws.onopen = () => setDebugBanner("OPEN — waiting for first message", "#3affc8");
  ws.onerror = () => setDebugBanner("error event", "#ff5d8f");
  ws.onclose = (e) => { setDebugBanner("CLOSED code=" + e.code + " — retrying in 2s", "#ff5d8f"); setTimeout(connect, 2000); };
  ws.onmessage = (ev) => {
    setDebugBanner("LIVE — receiving data", "#3affc8");
    try { _handleMessage(JSON.parse(ev.data)); }
    catch (err) {
      setDebugBanner("parse error: " + err.message, "#ff5d8f");
